 ...
 *
 * next/image runs through lib/imageLoader.ts, but a few places (the catalog
 * lightbox / cards) use native <img> for instant, dependency-free rendering.
 * Those bypass the loader and would otherwise download the full-resolution
 * original — the main cause of slow image loading, especially on iOS.
 *
 * `cdnImage(src, width, quality)` mirrors imageLoader: when the source lives on
 * a Cloudflare-resize-capable host (custom domain, NOT r2.dev / Supabase), it
 * rewrites to `/cdn-cgi/image/<opts>/<path>` so the edge serves a properly
 * sized, auto-format (WebP/AVIF) image. Otherwise the original URL is returned
 * untouched, so nothing ever breaks.
 */
